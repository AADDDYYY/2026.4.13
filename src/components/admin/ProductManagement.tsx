...
export default function ProductManagement() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      // Revert optimism? Simple reload here or just fetch again if it fails
    }
  };
  ...
  return (
    <div className="bg-white p-6 md:p-10 rounded-[32px] md:rounded-[40px] shadow-sm border border-brand-border mb-8 md:mb-12 relative overflow-hidden">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 md:mb-12 gap-6 md:gap-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl md:text-3xl font-black text-brand-dark">📦 产品上架管理</h2>
            <p className="text-brand-dark/40 text-[9px] md:text-[11px] font-black uppercase tracking-widest mt-1">Manage Catalog & Datasheets</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 md:gap-4">
          {!showSyncConfirm ? (
            <button 
              ...
              disabled={isSyncing}
              className="px-4 md:px-6 py-2 md:py-3 border-2 border-brand-blue text-brand-blue hover:bg-brand-blue hover:text-white text-[9px] md:text-[10px] font-black uppercase tracking-widest rounded-full transition-all flex items-center gap-2"
            >
              <RefreshCw className={`w-3 h-3 md:w-3.5 md:h-3.5 ${isSyncing ? "animate-spin" : ""}`} />
              {isSyncing ? '同步中' : '同步至云端'}
            </button>
          ) : (
            <div className="flex items-center gap-2 bg-brand-blue/10 p-1 rounded-full border border-brand-blue/20">
               <button 
                ...
                className="px-3 md:px-4 py-1.5 md:py-2 bg-brand-blue text-white text-[8px] md:text-[9px] font-black uppercase tracking-widest rounded-full hover:bg-brand-dark transition-all"
              >
                确认
              </button>
              <button 
                onClick={() => setShowSyncConfirm(false)}
                className="px-3 md:px-4 py-1.5 md:py-2 text-brand-dark/40 text-[8px] md:text-[9px] font-black uppercase tracking-widest rounded-full hover:bg-white transition-all"
              >
                取消
              </button>
            </div>
          ...
          <button 
            ...
            className="px-4 md:px-6 py-2 md:py-3 bg-brand-blue text-white text-[9px] md:text-[10px] font-black uppercase tracking-widest rounded-full hover:bg-brand-dark transition-all flex items-center gap-2 shadow-lg shadow-brand-blue/20"
          >
            <Plus className="w-3 h-3 md:w-3.5 md:h-3.5" /> 新增
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-brand-gray/30 rounded-2xl md:rounded-3xl border border-brand-border overflow-hidden">
